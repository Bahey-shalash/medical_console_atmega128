...
; we can use r12,r13,r14,r15 for other purposes
...
;----------------------------------------------------------------------
            ...
            .equ  FLG_TEMP   = 0          ; bit 0 in ‘flags’ = temperature-ready flag
            .equ  REG_STATES = 4          ; number of “game” states (0..3)
            .equ  ST_HOME    = 0         ; state indices
            .equ  ST_GAME1   = 1
            .equ  ST_GAME2   = 2
            .equ  ST_GAME3   = 3
            .equ  ST_DOCTOR  = 4
            ...
            .equ  LED_BIT    = 7          ; PF7 heartbeat(blinking) led that indicates
                                          ; that the timer overflows

;----------------------------------------------------------------------
;Reserves four bytes in SRAM:
;	•	flags: status flags, including temperature-ready.
;	•	temp_lsb / temp_msb: raw temperature bytes.
;	•	phase: toggles between “issue convert” and “read result.”
;
.dseg
...
;======================================================================
            .org 0
            jmp  reset  ; on reset → start here

            .org INT0addr       ; button “next state”
            jmp  int0_isr

            .org INT1addr        ; button “prev state”
            jmp  int1_isr      

            .org INT2addr        ; button “home”(reset to Home)
            jmp  int2_isr

            .org INT3addr        ; button “doctor mode”
            jmp  int3_isr

            .org OVF1addr        ; Timer1 overflow → heartbeat & temp task
            jmp  t1_isr
...
;� Buttons PD0�PD3: inputs w/ pull-ups -------------------------------
            ...
            cbi   DDRD,1
            cbi   DDRD,2
            cbi   DDRD,3             ; PD0–PD3 = inputs
            sbi   PORTD,0
            sbi   PORTD,1
            sbi   PORTD,2
            sbi   PORTD,3       ; enable pull-ups
            rcall ws_init       ; init WS2812 driver
